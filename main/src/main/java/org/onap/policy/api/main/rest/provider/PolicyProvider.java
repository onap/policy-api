...
public class PolicyProvider {
    ...
    public Map<Pair<String, String>, List<ToscaPolicy>> fetchDeployedPolicies(
        ...
        policies.add(new ToscaPolicyIdentifier(policyId, null));
        ...
        if (pdpGroups.isEmpty()) {
            throw new PfModelException(Response.Status.NOT_FOUND,
                    "policy with ID " + policyId + " has not been deployed yet");
        }

        close();
        return constructDeployedPolicyMap(pdpGroups, policyId);
    }
    ...
    public ToscaServiceTemplate deletePolicy(String policyTypeId, String policyTypeVersion,
        ...
        if (deletedServiceTemplate == null) {
            throw new PfModelException(Response.Status.NOT_FOUND,
                    constructResourceNotFoundMessage(policyTypeId, policyTypeVersion, policyId, policyVersion));
        }
        ...
        for (PdpGroup pdpGroup : pdpGroups) {
            ...
            if (deployedPolicies.isEmpty()) {
                throw new PfModelException(Response.Status.INTERNAL_SERVER_ERROR,
                        "returned pdp group with ID " + pdpGroup.getName() + ":" + pdpGroup.getVersion()
                      + " does not contain specified policy with ID " + policyId);
            }
    ...
    private String constructResourceNotFoundMessage(String policyTypeId, String policyTypeVersion,
        ...
        StringBuilder builder = new StringBuilder().append("policy with ID ").append(policyId)
                .append(":").append(policyVersion).append(" and type ").append(policyTypeId).append(":")
                .append(policyTypeVersion).append(" cannot be found");

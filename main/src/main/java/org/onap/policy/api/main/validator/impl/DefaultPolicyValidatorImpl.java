...
public class DefaultPolicyValidatorImpl implements PolicyValidator {
    ...
    public void validateNativePolicies(final List<Map<String, ToscaPolicy>> policies) throws PfModelException {
        ...
        validateDroolsArtifacts(nativePolicies);
    ...
    @Override
    public void validateDroolsArtifacts(final List<Map<String, ToscaPolicy>> droolsPolicies) throws PfModelException {

        for (Map<String, ToscaPolicy> policy : droolsPolicies) {
            validateDroolsArtifact(policy.entrySet().iterator().next().getValue(), client);
        }
    }
     ...
     */
    public void validateDroolsArtifact(final ToscaPolicy droolsPolicy, HttpClient client) throws PfModelException {
        ...
        StandardCoderObject propertyObject;
        try {
            propertyObject = coder.toStandard(droolsPolicy.getProperties().get(PROPERTY_RULE_ARTIFACT));
        ...
        String path = "resolve?r=" + RELEASE + "&g=" + propertyObject.getString(GROUP_ID)
                + "&a=" + propertyObject.getString(ARTIFACT_ID) + "&v=" + propertyObject.getString(VERSION);

        try {
            if (!NetworkUtil.isTcpPortOpen(
                    parameters.getNexusName(), Integer.valueOf(parameters.getNexusPort()), 5, 100L)) {
                throw new PfModelException(Response.Status.INTERNAL_SERVER_ERROR,
                        "cannot connect to " + parameters.getNexusName() + ":" + parameters.getNexusPort());
            }
        } catch (Exception exc) {
            throw new PfModelException(Response.Status.INTERNAL_SERVER_ERROR,
                    "errors on connecting to " + parameters.getNexusName() + ":" + parameters.getNexusPort(), exc);
        }
        ...
        switch (code) {
            case 404:
                String errMsg = "rule artifact " + propertyObject.getString(GROUP_ID) + "|"
                     + propertyObject.getString(ARTIFACT_ID) + "|" + propertyObject.getString(VERSION) + " not found";

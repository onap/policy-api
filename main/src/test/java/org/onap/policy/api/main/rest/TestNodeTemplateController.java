...
public class TestNodeTemplateController extends CommonTestRestController {
    ...
    public void testCreateToscaNodeTemplates() throws Exception {
        Response rawResponse = createResource(getUrl(NODE_TEMPLATES), TOSCA_NODE_TEMPLATE_RESOURCE, apiPort);
        ...
        rawResponse = createResource(getUrl(NODE_TEMPLATES), TOSCA_INVALID_NODE_TYPE, apiPort);
        ...
        rawResponse = createResource(getUrl(NODE_TEMPLATES), TOSCA_INVALID_TEMPLATE, apiPort);
    ...
    public void testReadNodeTemplates() throws Exception {
        Response rawResponse = readResource(getUrl(NODE_TEMPLATES), APP_JSON, apiPort);
        assertEquals(Response.Status.OK.getStatusCode(), rawResponse.getStatus());
        List<?> nodeTemplates = rawResponse.readEntity(List.class);
        assertNotNull(nodeTemplates);
        assertEquals(0, nodeTemplates.size());

        createResource(getUrl(NODE_TEMPLATES), TOSCA_NODE_TEMPLATE_RESOURCE, apiPort);
        rawResponse = readResource(getUrl(NODE_TEMPLATES), APP_JSON, apiPort);
        assertEquals(Response.Status.OK.getStatusCode(), rawResponse.getStatus());
        nodeTemplates = rawResponse.readEntity(List.class);
        assertNotNull(nodeTemplates);
        assertEquals(3, nodeTemplates.size());

        rawResponse = readResource(getUrl(SPECIFIC_NODE_TEMPLATE), APP_JSON, apiPort);
        assertEquals(Response.Status.OK.getStatusCode(), rawResponse.getStatus());
    ...
    public void testUpdateNodeTemplates() throws Exception {
        createResource(getUrl(NODE_TEMPLATES), TOSCA_NODE_TEMPLATE_RESOURCE, apiPort);
        Response rawResponse = updateResource(getUrl(NODE_TEMPLATES), TOSCA_UPDATE_NODE_TEMPLATES, APP_JSON, apiPort);
        assertEquals(Response.Status.OK.getStatusCode(), rawResponse.getStatus());
        ToscaServiceTemplate response = rawResponse.readEntity(ToscaServiceTemplate.class);
        assertNotNull(response);
        assertFalse(response.getToscaTopologyTemplate().getNodeTemplates().isEmpty());
        ...
        rawResponse = updateResource(getUrl(NODE_TEMPLATES), TOSCA_INVALID_NODE_TYPE, APP_JSON, apiPort);
        assertEquals(Response.Status.NOT_ACCEPTABLE.getStatusCode(), rawResponse.getStatus());
        ErrorResponse response2 = rawResponse.readEntity(ErrorResponse.class);
        assertThat(response2.getErrorMessage())
            .containsPattern("^NODE_TYPE .* for toscaNodeTemplate .* does not exist$");
        ...
        // Send invalid tosca template with no node templates
        rawResponse = updateResource(getUrl(NODE_TEMPLATES), TOSCA_INVALID_TEMPLATE, APP_JSON, apiPort);
        assertEquals(Response.Status.NOT_FOUND.getStatusCode(), rawResponse.getStatus());
    ...
    public void testDeleteNodeTemplates() throws Exception {
        createResource(getUrl(NODE_TEMPLATES), TOSCA_NODE_TEMPLATE_RESOURCE, apiPort);
        Response rawResponse = deleteResource(getUrl(SPECIFIC_NODE_TEMPLATE), APP_JSON, apiPort);
        assertEquals(Response.Status.OK.getStatusCode(), rawResponse.getStatus());
        ToscaServiceTemplate response = rawResponse.readEntity(ToscaServiceTemplate.class);
        assertNotNull(response);
        assertFalse(response.getToscaTopologyTemplate().getNodeTemplates().isEmpty());
        ...
        // Send invalid id
        rawResponse = deleteResource(getUrl(INVALID_NODE_TEMPLATE_ID), APP_JSON, apiPort);
        assertEquals(Response.Status.NOT_FOUND.getStatusCode(), rawResponse.getStatus());
        ErrorResponse response3 = rawResponse.readEntity(ErrorResponse.class);
    ...
    private String getUrl(String endpoint) {
        return "https://localhost:" + apiPort + "/nodetemplate/api/v1/" + endpoint;
    }
